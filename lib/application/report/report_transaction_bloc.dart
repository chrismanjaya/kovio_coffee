...
import 'package:kovio_coffee/domain/model/transaction_report_status_model.dart';
  ...
  Stream<ReportTransactionState> mapEventToState(
    ...
    yield* event.map(
      getReport: (e) async* {
        ...
        // Get Sold Product Report
        final getAllSoldProductByFilter = await _transactionReportServices
            .getAllSoldProductByFilter(e.transactionFilterModel);
        if (getAllSoldProductByFilter.isRight()) {
          yield state.copyWith(
            isLoading: false,
            listSoldProductReportModel:
                getAllSoldProductByFilter.fold((l) => [], (data) => data),
            errorMessage: "",
          );
        } else {
          yield state.copyWith(
            isLoading: false,
            errorMessage: getAllSoldProductByFilter.fold(
                (failure) => failure, (r) => null),
          );
        }
      },
