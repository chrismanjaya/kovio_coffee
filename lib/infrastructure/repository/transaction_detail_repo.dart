...
import 'package:kovio_coffee/infrastructure/failure/firebase_firestore_failure.dart';
...
class TransactionDetailRepo {
  ...
  Future<Either<String, List<TransactionDetail>>>
      getListTransactionDetailByListTransactionId(
          List<String> listTransactionId) async {
    try {
      var batch = [];
      for (var i = 0; i < listTransactionId.length; i += 10) {
        batch.add(listTransactionId.sublist(
            i,
            i + 10 > listTransactionId.length
                ? listTransactionId.length
                : i + 10));
      }
      List<TransactionDetail> listTransactionDetail = [];
      for (List<String> ids in batch) {
        final QuerySnapshot transactionDetailCollection =
            await _transactionDetailRef
                .where("transactionId", whereIn: ids)
                .orderBy("itemNo", descending: false)
                .get();
