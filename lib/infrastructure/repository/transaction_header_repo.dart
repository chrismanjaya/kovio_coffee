...
class TransactionHeaderRepo {
  ...
  Future<Either<String, List<TransactionHeader>>>
    ...
    try {
      ...
      DateTime dateFrom = activityFilterModel.dateFrom;
      DateTime dateTo = activityFilterModel.dateTo.add(const Duration(days: 1));
      final QuerySnapshot transactionHeaderCollection =
          await _transactionHeaderRef
              .where("branchName", whereIn: listBranchName)
              .where("transactionDate", isGreaterThanOrEqualTo: dateFrom)
              .where("transactionDate", isLessThan: dateTo)
              .orderBy("transactionDate", descending: true)
              .get();
